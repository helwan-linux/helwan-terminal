# تنظيف الحروف الخفية تلقائياً عند التشغيل
sed -i 's/\xC2\xA0/ /g' "$0" 2>/dev/null

# تحميل ملف الـ bashrc الأصلي للمستخدم أولاً لو موجود عشان تظل إعداداته وaliases الأساسية شغالة
if [ -f ~/.bashrc ]; then
    source ~/.bashrc
fi
...
search-local() { helwan_search_local "$@"; }
buscar-local() { helwan_search_local "$@"; }
...
pkg-info() { helwan_pkg_info "$@"; }
info-paquete() { helwan_pkg_info "$@"; }
...
local-install() { helwan_local_install "$@"; }
instalar-local() { helwan_local_install "$@"; }
...
clr-cache() { helwan_clr_cache; }
limpiar-caché() { helwan_clr_cache; }
...
# ==========================================
# 12. إزالة الحزم اليتيمة والمخلفات (autoremove)
# ==========================================
helwan_autoremove() {
    sudo pacman -Rns "$@"
}
...
limpiar-automatico() { helwan_autoremove "$@"; }
